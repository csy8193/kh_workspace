	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("button.btnOK").click(function() {
				...
				// *** (요구사항2) *** //
				// form 태그의 name이 commentFrm 인 모든 입력값들을 하나의 문자열로 받아 변수명 queryString 에 저장시킨다.
				...
				$.ajax({
					...
					//--------------------------
					url :"/swtest/ncstest/commentRegister.do",
				  	type : "POST",
				  	data : queryString,								
		...
		function func_init() {
			$.ajax({
				...
				// jQuery를 사용한 Ajax로 처리하도록 한다.
				// URL 주소는 commentList.do 이며 GET 방식으로 JSON 형태로 데이터를 받아서 처리토록 한다. 
				// 웹페이지에 보여지는 위치는 div 태그의 id 값이 viewComments 인 곳이다.
				// 만약에 등록된 댓글내용이 없을 경우에는 "데이터가 없습니다." 라고 나타내도록 한다.  	
				
				//------------------------------------------
				url:"/swtest/ncstest/commentList.do",
				type:"GET",
				dataType:"json",
